...
#Now, save 100 posts from Barack Obama's page into obama_page variable.
obama_page = getPage("barackobama",token,n = 100)

#Get the the first post.
obama_post = getPost(post = obama_page$id[1],n = 100, token)

#Get the replies to the comments.
obama_message = (obama_post$comments)$message

#Create the corpous after passing docs into VectorSource().
docs = Corpus(VectorSource(obama_message))
